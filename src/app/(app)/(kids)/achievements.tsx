// src/app/(app)/(kids)/achievements.tsx
// UC037: Conquistas da criança
...
/** Mapa de nome de ícone Tabler para componente. Estender conforme necessário. */
...
function BadgeIcon({ name, size = 26, color }: { name: string; size?: number; color: string }) {
  const Icon = TABLER_ICON_MAP[name] ?? IconBolt;
...
function AchievementCard({ item, unlocked }: { item: ChildAchievement | typeof ACHIEVEMENT_MILESTONES[0]; unlocked: boolean }) {
  ...
  const iconName = isAchievement
    ? (item as ChildAchievement).badge_icon
    : (item as typeof ACHIEVEMENT_MILESTONES[0]).icon;
  const title = item.title;
  const description = item.description ?? '';
  const date = isAchievement ? new Date((item as ChildAchievement).awarded_at).toLocaleDateString('pt-BR', { day: 'numeric', month: 'short' }) : null;
    ...
    >
      ...
      <View style={{ flex: 1 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: Spacing.sm }}>
          <Text style={{ color: Colors.text, fontSize: FontSize.base, fontWeight: '600', flex: 1 }}>{title}</Text>
          {unlocked && date ? (
...
export default function AchievementsScreen() {
  ...
  const { summaries, achievements, loading, loadForChild, refresh } = useKids(childId);
  ...
  // Quais marcos já foram desbloqueados
  const unlockedPoints = new Set(achievements.map((a) => a.points_at));

  // Conquistas manuais (não-marcos)
  ...
  return (
    <View style={{ flex: 1, backgroundColor: Colors.bg }}>
      ...
      >
        ...
        <View style={{ flex: 1 }}>
          <Text style={{ color: Colors.text, fontSize: FontSize.xl, fontWeight: '600' }}>conquistas</Text>
          {child && (
            ...
            {achievements.length === 0 && (
              <View style={{ alignItems: 'center', paddingVertical: 32 }}>
                ...
                <Text style={{ color: Colors.muted, fontSize: FontSize.base, textAlign: 'center' }}>
                  nenhuma conquista ainda. complete tarefas para desbloquear!
                </Text>
              </View>
            )}
          </>
